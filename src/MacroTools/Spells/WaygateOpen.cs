...
namespace MacroTools.Spells
{
  ...
  public sealed class WaygateOpen : Spell
  {
    ...
    public override void OnCast(unit caster, unit target, Point targetPoint)
    {
      ...
      _exteriorWaygate = CreateUnit(Player(PLAYER_NEUTRAL_PASSIVE), ExteriorWaygateUnitTypeId, exteriorWaygatePosition.X, exteriorWaygatePosition.Y, 0);
      _exteriorWaygate.SetWaygateDestination(interiorWaygatePosition);

      _interiorWaygate = CreateUnit(Player(PLAYER_NEUTRAL_PASSIVE), InteriorWaygateUnitTypeId, interiorWaygatePosition.X, interiorWaygatePosition.Y, 0);
